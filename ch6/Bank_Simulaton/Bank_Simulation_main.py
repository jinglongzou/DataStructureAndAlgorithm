#####################
#银行柜台办理与海关检查的模拟类似
#变化的仅仅是当某柜台的业务员空闲，且其等待队列已空时，他需要从其他等待队列叫一个顾客
#那么他从其他非空的等待队列叫顾客的规则如何设定呢？从等待之人中最先到的，也就是比较非空等待队列的队首顾客
#取到达时间最小的一个
###############################
...
class Leave(Event):
    ...
    def run(self):
        ...
        if customs.has_queued_car(self.gate_index):   #检查当前通道对应的等待队列是否有车
            ...
            #从非空等待队列中选最早到达的顾客
            car = customs.waitline.dequeue_earliest_car()
            if car != None:
                i = customs.find_gate([self.gate_index])
                event_log(time, 'car check')
                customs.wait_time_acc(time - car.arrive_time())
                Leave(time + randint(*customs.check_interval), self.gate_index, car, customs)
        print('total_wait_time: ' + str(customs.total_wait_time) + ' | ' + 'total_used_time: ' + str(
            customs.totol_used_time))
